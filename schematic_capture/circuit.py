...
from schematic_capture.net import Net, SubNet
...
class Circuit:
    ...
    def __init__(self, netlist: Netlist, topology_layer=1, name=""):
        """Initialize a circuit for Netlist <netlist>.
    ...
    def generate_circuit_graph(self, excluded_nets: tuple[Net, ...] = ()):
        """Generate a graph for the circuit.

        Args:
            excluded_nets (tuple[Net, ...] , optional): Nets, which shall be excluded in the graph. Defaults to ().

        """
        self._graph = nx.MultiGraph()
        self._graph.add_nodes_from(self._gen_nodes_list_from_devices())
        self._graph.add_edges_from(
            self._gen_edge_list_from_net(excluded_nets=excluded_nets)
        )

    def update_circuit_graph(self, excluded_nets: tuple[Net, ...] = ()):
        """Update the graph for the circuit.

        Args:
            excluded_nets (tuple[Net, ...] , optional): ets, which shall be excluded in the graph.. Defaults to ().
    ...
    def feature_graph(self) -> nx.MultiGraph:
        ...
        for n, n_attr in self.graph.nodes(data=True):
            node_list.append((
                n,
                {"x": n_attr["Device"].feature_list, "name": n_attr["Device"].name},
            ))
        ...
        for u, v, e_attr in self.graph.edges(data=True):
            edge_list.append((
                ...
                v,
                {
                    "edge_attr": e_attr["Net"].feature_list_between(
                        self.devices[u], self.devices[v]
                    ),
                    "edge_name": e_attr["Net"].name,
                },
            ))
    ...
    def get_bipartite_graph(self, exclude_nets: tuple[Net, ...] = ()) -> nx.MultiGraph:
        ...
        Args:
            exclude_nets (tuple[Net, ...], optional): Nets which shall be excluded in the graph. Defaults to ().
        ...
        bipartite_graph.add_nodes_from(node_list_devices, bipartite=0)
        bipartite_graph.add_nodes_from(node_list_nets, bipartite=1)
        bipartite_graph.add_edges_from(edge_list)

        return bipartite_graph

    def include_primitives(
        self, primitives: dict[str, list[PrimitiveDeviceComposition]]
    ):
        ...
        """
        for _, prim_list in primitives.items():
            ...
            for primitive in prim_list:
                ...
                for device in primitive.devices:
                    ...
                    for _, net in device.nets.items():
    ...
    def map_devices_to_netlist(self) -> dict[str, Device]:
        ...
        """
        device_map = {}
        for _, device_ in self.devices.items():
            d_name_without_suffix = device_.name_without_suffix
            device_map[d_name_without_suffix] = device_
    ...
    def _instantiate_devices(self):
        ...
        for line_ in self._netlist.get_net():
            device = None
            ...
            # generate devices
            if line_.startswith("XM"):  # mosfet
                device = MOS(line_, name_suffix=name_suffix)
            elif line_.startswith("XR"):  # resistor
                device = ThreeTermResistor(line_, name_suffix=name_suffix)
            elif line_.startswith("XC"):  # capacitor
                device = Capacitor(line_, name_suffix=name_suffix)
            elif line_.startswith("x"):  # sub-device
                # get the model of the sub-device
                device_model = SubDevice.get_model(line_)
                ...
                device = SubDevice(
                    line_, name_suffix=name_suffix, terminal_names=terminal_names
                )
    ...
    def _instantiate_nets(self):
        ...
        for _, device_ in self._devices.items():
            # iterate over the nets of the device
            for net_name_, net_ in device_.nets.items():
                if net_ is None:  # if net isn't initialized
                    # check if the net is already instantiated in the circuit
                    if net_name_ in self._nets:
                        # retrieve the net
                        new_net = self._nets[net_name_]
                    else:
                        # generate a new net
                        if isinstance(self, SubCircuit):
                            # if the circuit is a SubCircuit, generate a SubNet
                            new_net = SubNet(net_name_, self, self.sub_device)
                        else:
                            # generate a top-net
                            new_net = Net(net_name_, self)

                        # add the net to the nets dict
                        self._nets[net_name_] = new_net

                    new_net.add_device(device_)  # add actual device to net
                    device_.set_net_class(new_net)  # set net for device

            nterm_device: NTermDevice = device_  # Check: Can I change type this way?
            # set the nets of the terminals
            for k, v in nterm_device.terminal_nets.items():
                nterm_device.terminals[k].set_net(v)

    def _gen_edge_list_from_net(
        self, excluded_nets: tuple[Net, ...] = ()
    ) -> list[tuple]:
        """Generate the edge list from the circuits nets.

        Args:
            excluded_nets (tuple[Net, ...], optional): Nets which shall be excluded. Defaults to ().
        ...
        for net_name, n in self._nets.items():
            # if the net isn't excluded
            if net_name not in excluded_nets:
                ...
                comb = Circuit._get_combinations(d, attr={"Net": n})
    ...
    def _gen_nodes_list_from_nets(
        self, exclude_nets: tuple[Net, ...] = ()
    ) -> list[tuple[str, dict]]:
    ...
    def _gen_edge_list_from_devices(
        self, exclude_nets: tuple[Net, ...] = ()
    ) -> list[tuple[str, str, dict]]:
        """Generate  edges from a devices, to their connected nets.

        Args:
            exclude_nets (tuple[Net, ...], optional): Nets which shall be excluded. Defaults to ().
            ...
            for n_name, n in d.get_nets().items():
                if n_name in exclude_nets:
                    ...
                    edge_list.append((
                        d_name,
                        n_name,
                        {"Terminal": "".join(d.map_nets_to_terminal_names(n))},
                    ))
        return edge_list

    @staticmethod
    def _get_combinations(input_list: list, attr=None):
        """Get combinations of the items in the list.

        Args:
            input_list (list): List which items have to be combinized.
        ...
        Example:
            c = _get_combinations(l=[1,2,3])
            c = [(1,2),(1,3),(2,3)]
            c = _get_combinations(l=[1,2,3], attr='example')
        ...
        for i in range(len(input_list) - 1):
            ...
            for j in range(i + 1, len(input_list)):
                ...
                if attr:
                    result.append((input_list[i], input_list[j], attr))
                else:
...
class SubCircuit(Circuit):
    ...
    def terminal_nets(self) -> OrderedDict[str, Net]:
        """Get a map of the terminal nets to internal net instances.
    ...
    def get_terminal_names(line: str) -> list[str]:
        ...
        """
        assert line.startswith(".subckt") or line.startswith(".SUBCKT"), (
            "Spice line for subckt definition don't starts with .subckt!"
        )
