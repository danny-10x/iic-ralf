...
class Device(metaclass=abc.ABCMeta):
    ...
    def __init__(self, spice_description: str, name_suffix=""):
        ...
        self._nets = (
            OrderedDict()
        )  # nets connected to the device, ordered according the spice-description
        ...
        self._terminals = (
            OrderedDict()
        )  # terminals of the device, ordered according the spice-description
        ...
        self._name = (
            self._spice_splitted[0] + "_" + name_suffix
            if name_suffix != ""
            else self._spice_splitted[0]
        )
    ...
    def set_net_class(self, net: Net):
        ...
        """
        assert net.name in self._nets, (
            f"Netname {net.name}, not registered in the devices nets!"
        )
    ...
    def get_nets_and_terminals(self) -> dict[str, tuple[Net, Pin]]:
        ...
        """
        temp = {}
        for net_name, net, terminal in zip(
            self._nets.keys(), self._nets.values(), self._terminals.values()
        ):
    ...
    def _get_params(line: str) -> list[str]:
        """Get parameters from spice description.
        ...
        for n in range(len(line)):
            ...
            if line_ == "'" and not ignore_space and param:
                ignore_space = True
                continue

            # if in param and a math statement ends
            if line_ == "'" and ignore_space and param:
...
class NTermDevice(Device, metaclass=abc.ABCMeta):
    ...
    def __init__(self, spice_description: str, N_Terminals: int, name_suffix=""):
        ...
        assert len(self._terminal_nets) == len(self._terminals), (
            f"Number of nets and terminals don't match for device {self.name}!"
        )
    ...
    def terminal_nets(self) -> dict[str, Net]:
        ...
        return {
            terminal_name: self._nets[self._terminal_nets[i]]
            for (terminal_name, i) in zip(
                self.terminals.keys(), range(len(self.terminals))
            )
        }
    ...
    def map_nets_to_terminals(self) -> dict[str, Pin]:
        ...
        """
        return dict(zip(self._terminal_nets, self._terminals.values()))
...
class PrimitiveDevice(NTermDevice, metaclass=abc.ABCMeta):
    ...
    def _generate_routing_rules(self) -> list[RoutingRule]:
        """Generate the routing rules for a primitive device.
...
class ThreeTermResistor(PrimitiveDevice):
    ...
    def __init__(self, spice_description: str, name_suffix=""):
        """Initialize a three-terminal resistor.
        ...
        if self._spice_splitted[4] not in SUPPORTED_DEVICES:
            raise ValueError(
                f"Device {self._spice_splitted[0]} of type {self._spice_splitted[4]} not supported!"
            )
...
class Capacitor(PrimitiveDevice):
    ...
    def __init__(self, spice_description: str, name_suffix=""):
        ...
        if self._spice_splitted[3] not in SUPPORTED_DEVICES:
            raise ValueError(
                f"Device {self._spice_splitted[0]} of type {self._spice_splitted[3]} not supported!"
            )
...
class MOS(PrimitiveDevice):
    ...
    def __init__(self, spice_description: str, name_suffix=""):
        ...
        if self._spice_splitted[5] not in SUPPORTED_DEVICES:
            raise ValueError(
                f"Device {self._spice_splitted[0]} of type {self._spice_splitted[5]} not supported!"
            )
    ...
    def _setup_terminals(self):
        """Set up the terminals of the MOS."""
        self._terminals["D"] = Pin("D", self)
...
class SubDevice(NTermDevice):
    ...
    def __init__(
        self, spice_description: str, terminal_names: list[str], name_suffix=""
    ):
        """Initialize a sub-device.
    ...
    def internal_nets(self) -> dict[str, list[Net]]:
        ...
        for internal_net_name, outer_net_name in zip(
            self._terminal_names, self._terminal_nets
        ):
    ...
    def _setup_terminals(self):
        """Set up terminals for the SubDevice.
    ...
    def _update_terminals_from_circuit(self):
        """Update the terminals dict.

        Such that the name of the terminal is equal
        to the terminal name specified in the sub-circuit definition.
        """
        new_terminal_dict = OrderedDict()
        for new_name, terminal in zip(
            self._circuit._terminal_net_keys, self._terminals
        ):
    ...
    def _set_terminal_child_nets(self):
        ...
        for term, inner_net in zip(
            self._terminals.values(), self._circuit.terminal_nets.values()
        ):
