...
def get_RStrings(
    ...
    for path in RSTRING_PATHS:
        ...
        for gm in GM.subgraph_isomorphisms_iter():
            ...
            rstring_edges.append(
                tuple(sorted([gm2[k] for k in primitive_circuit.devices]))
            )
        ...
        rstring_edges = list(
            filter(
                lambda x: circ.devices[x[0]].parameters["L"]
                == circ.devices[x[1]].parameters["L"]
                and circ.devices[x[0]].terminal_nets["B"]
                == circ.devices[x[1]].terminal_nets["B"],
                rstring_edges,
            )
        )
    ...
    components = [
        rstring_graph.subgraph(c).copy() for c in nx.connected_components(rstring_graph)
    ]
    ...
    def __init__(self, devices: list[ThreeTermResistor], use_dummies=True):
        ...
        for device, i in zip(self._devices, range(len(self._devices))):
            ...
            if i > 0:
                ...
                if (
                    dev_nets[0] == last_net_order[1]
                ):  # if the first and second net are common, change the order
                    dev_nets[0], dev_nets[1] = dev_nets[1], dev_nets[0]
                elif (
                    dev_nets[1] == last_net_order[0]
                ):  # if the second and first net are common, change the order
                    dev_nets[0], dev_nets[1] = dev_nets[1], dev_nets[0]
