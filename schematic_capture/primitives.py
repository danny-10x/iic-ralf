...
from schematic_capture.devices import SUPPORTED_DEVICES, PrimitiveDevice
...
class PrimitiveDeviceComposition(PrimitiveDevice, metaclass=abc.ABCMeta):
    ...
    def __init__(
        self,
        devices: list[PrimitiveDevice],
        spice_description: str,
        n_terminals: int,
        name_suffix="",
        use_dummies=False,
    ):
        """Initialize a primitive device composition.
        ...
        Args:
            ...
            use_dummies (bool): If the device shall use dummies. Defaults to False.
        ...
        for device in devices:
            assert device.model == model, (
                "Primitive device composition, with unequal models detected!"
            )
...
class DifferentialPair(PrimitiveDeviceComposition):
    ...
    def __init__(self, devices: list[PrimitiveDevice], use_dummies=False):
        ...
        super().__init__(
            devices=devices,
            spice_description=spice_description,
            n_terminals=n_terminals,
            name_suffix=self._d1._name_suffix,
    ...
    def _setup_terminals(self):
        """Set up the terminal pins of the device.
...
class DifferentialLoad(PrimitiveDeviceComposition):
    ...
    def __init__(self, devices: list[PrimitiveDevice], use_dummies=False):
        """Initialize a differential load.
        ...
        param_l = self._d1.parameters["L"]

        if self._d1.parameters["W"] != self._d2.parameters["W"]:
            raise ValueError("Differential load with unequal W detected!")

        param_w = self._d1.parameters["W"]

        if self._d1.parameters["nf"] != self._d2.parameters["nf"]:
            raise ValueError("Differential load with unequal L detected!")

        param_nf = self._d1.parameters["nf"]
        ...
        if use_dummies:
            ...
            # add dummy fingers
            spice_description = (
                f"XDL_{self._d1.name_without_suffix}_{self._d2.name_without_suffix}"
                f" {nets1['D'].name} {nets2['D'].name} {nets1['S'].name} {nets2['S'].name}"
                f" {nets1['G'].name} {nets1['B'].name} W={param_w} L={param_l} nf={param_nf + 2} m={2}"
            )
        else:
            spice_description = (
                f"XDL_{self._d1.name_without_suffix}_{self._d2.name_without_suffix}"
                f" {nets1['D'].name} {nets2['D'].name} {nets1['S'].name} {nets2['S'].name}"
                f" {nets1['G'].name} {nets1['B'].name} W={param_w} L={param_l} nf={param_nf} m={2}"
            )
    ...
    def _setup_terminals(self):
        """Set up the terminals of the differential load.
...
class CrossCoupledPair(PrimitiveDeviceComposition):
    ...
    def __init__(self, devices: list[PrimitiveDevice], use_dummies=False):
        """Initialize a cross coupled pair.
        ...
        if use_dummies:
            spice_description = (
                f"XCCP_{self._d1.name_without_suffix}_{self._d2.name_without_suffix}"
                f" {nets1['D'].name} {nets2['D'].name} {nets1['S'].name} {nets2['S'].name}"
                f" {nets1['B'].name} W={param_w} L={param_l} nf={param_nf + 2} m={2}"
            )
        else:
            spice_description = (
                f"XCCP_{self._d1.name_without_suffix}_{self._d2.name_without_suffix}"
                f" {nets1['D'].name} {nets2['D'].name} {nets1['S'].name} {nets2['S'].name}"
                f" {nets1['B'].name} W={param_w} L={param_l} nf={param_nf} m={2}"
            )
    ...
    @property
    def devices(self):
        """Return devices in the cross coupled pair."""
        return self._devices
