...
class RectanglePackingProblemAnnealerHard(RectanglePackingProblemAnnealer):
    ...
    def move(self) -> float:
        ...
        for _ in range(10000):
            ...
            i, j = random.sample(
                range(self.problem.n), k=2
            )  # The first and second index
            offset = (
                random.randint(0, 1) * self.problem.n
            )  # Choose G_{+} (=0) or G_{-} (=1)

            # Swap them (i != j always holds true)
            self.state[i + offset], self.state[j + offset] = (
                initial_state[j + offset],
            ...
            if self.problem.rectangles[i]["rotatable"]:
                if random.randint(0, 1) == 1:
                    self.state[i + 2 * self.problem.n] = (
                        initial_state[i + 2 * self.problem.n] + 1
                    )
...
class HardToFindSolutionError(Exception):
