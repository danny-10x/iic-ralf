...
class MagicParser:
    ...
    def get_layers(self, lines: list[str]) -> dict[str, MagicLayer]:
        ...
        while n < len(lines):
            ...
            if MagicParser.get_layer(line_):
                ...
                layer = MagicLayer(MagicParser.get_layer(line_), color)
                n += 1
    ...
    def get_rect(self, line: str) -> Rectangle | None:
        ...
        if line.startswith("rect"):
            line_ = line.split()
            return Rectangle(
                int(line_[1]) / self._magscale,
                int(line_[2]) / self._magscale,
                int(line_[3]) / self._magscale,
                int(line_[4]) / self._magscale,
            )
        else:
            return None
