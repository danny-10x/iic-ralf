...
class Rectangle:
    ...
    def move(self, coordinate: tuple[int | float, int | float]):
        r"""Move the rectangle by the amount of <coordinate>.
    ...
    def rotate(self, mean_coordinate: tuple[int | float, int | float], angle: int):
        """Rotate the rectangle around <mean_coordinate> counter-clockwise, by <angle>.
    ...
    def overlap(rect1: Rectangle, rect2: Rectangle) -> bool:
        ...
        Args:
            rect1 (Rectangle): 1st rectangle
            rect2 (Rectangle): 2nd rectangle

        Returns:
            bool: True if the rectangles overlap

        """
        bound1 = rect1.get_coordinates()
        bound2 = rect2.get_coordinates()
    ...
    @staticmethod
    def touching(rect1: Rectangle, rect2: Rectangle) -> bool:
    ...
    @staticmethod
    def is_inside(rect1: Rectangle, rect2: Rectangle) -> bool:
        ...
        Returns:
            bool: True if rect1 is inside rect2 (including edges).
    ...
    def __eq__(self, __value: object) -> bool:
        """Override default behaviour."""
        return (
            isinstance(__value, Rectangle) and self.bounding_box == __value.bounding_box
        )
...
class MagicLayer:
    ...
    def move(self, coordinate: tuple[float | int, float | int]):
        """Move the rectangles of the layer by the amount given in <coordinate>.
    ...
    def draw(self, surface: pygame.Surface):
        ...
        for r in self._rects:
            pygame.draw.rect(
                surface,
                self._color.rgb,
                copy.copy(r.to_pygame()),
                0 if self._fill else 5,
            )
    ...
    @staticmethod
    def get_overlaps(layer1: Layer, layer2: Layer) -> list[Rectangle]:
        ...
        rects_layer1 = layer1.rectangles
        rects_layer2 = layer2.rectangles

        for ri in rects_layer1:
            for rj in rects_layer2:
