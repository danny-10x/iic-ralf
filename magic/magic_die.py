...
if TYPE_CHECKING:
    ...
    from lef_def_parser.def_util import Pin
...
class MagicDie:
    ...
    def __init__(self, circuit: Circuit, def_file: str | None = None) -> None:
        """Initialize a die.
        ...
        if def_file is not None:
            try:
                ...
                assert (
                    self._def_parser.units == "MICRONS"
                    f"Unsupported unit {self._def_parser.units} detected!"
                )
                self._def_scale = self._def_parser.scale

            except Exception as e:
                raise ValueError(f"Parsing of def file {def_file} failed!") from e
        else:
    ...
    def _get_bounding_box(self) -> Rectangle:
        ...
        """
        if self._def_parser is not None:
            parser_area = self._def_parser.diearea
            parser_scale = (
                (int(self._def_parser.scale) / 1000) * INTERNAL_SCALE
            )  # convert the scale to nm and multiply with the internal scale

            return Rectangle(
                parser_area[0][0] / parser_scale,
                parser_area[0][1] / parser_scale,
                parser_area[1][0] / parser_scale,
                parser_area[1][1] / parser_scale,
            )
        else:
            return None

    def _setup_pins_and_nets(self):
        """Set up the io-pins and the io-nets.
        ...
        if self._def_parser is not None:
            ...
            for pin in self._def_parser.pins:
                ...
                layer_points = (
                    layer_points[0][0] / parser_scale,
                    layer_points[0][1] / parser_scale,
                    layer_points[1][0] / parser_scale,
                    layer_points[1][1] / parser_scale,
                )

                pin_placed_at = pin.placed  # center-point of the pin
                # convert the coordinates to the internal scale
                pin_placed_at = (
                    pin_placed_at[0] / parser_scale,
                    pin_placed_at[1] / parser_scale,
                )
                # generate a rectangle for the pin
                pin_rectangle = Rectangle(
                    layer_points[0] + pin_placed_at[0],
                    layer_points[1] + pin_placed_at[1],
                    layer_points[2] + pin_placed_at[0],
                    layer_points[3] + pin_placed_at[1],
                )
                ...
                try:
                    ...
                    print(
                        f"Net {net} couldn't be found in the circuit! Probably unconnected net. Adding new net to die."
                    )
                ...
                try:
                    ...
                    raise ValueError(
                        f"Layer {layer_name} couldn't be found in the PDK!"
                    ) from e
...
class MagicDiePin:
    ...
    def __init__(
        ...
        layer (Layer): layer.
        net (Net): net,
        bounding_box: (Optional[Rectange]): Specify pin bounding box if needed.
    ...
    def get_bounding_box_on_grid(self) -> Rectangle:
        ...
        x_min = math.floor(bound[0] / 2.0) * 2
        y_min = math.floor(bound[1] / 2.0) * 2
        x_max = math.ceil(bound[2] / 2.0) * 2
        y_max = math.ceil(bound[3] / 2.0) * 2
    ...
    def get_coordinate_on_grid(self) -> tuple[int, int]:
        ...
        x = round(coordinate[0] / 2.0) * 2
        y = round(coordinate[1] / 2.0) * 2
    ...
    def __eq__(self, __value: object) -> bool:
        ...
        return (
            isinstance(__value, MagicDiePin)
            and (self._x == __value._x)
            and (self._y == __value._y)
            and (self._layer == __value.layer)
            and (self._net == __value._net)
        )
