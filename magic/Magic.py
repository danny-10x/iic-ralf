...
if TYPE_CHECKING:
    ...
    from schematic_capture.devices import MOS, Capacitor, ThreeTermResistor
...
class Magic:
    ...
    def __init__(self, circuit: Circuit):
        """Initialize the Magic class, to generate commands for the circuit.
    ...
    def gen_sky130_fd_pr__nfet_01v8(d: MOS) -> str:
        ...
        # get the parameters
        param_w = d.parameters["W"]
        param_l = d.parameters["L"]
        param_nf = d.parameters["nf"]
        param_m = d.parameters["m"]
        ...
        w_per_finger = round(param_w / param_nf, 2)

        # generate the command
        command = (
            "::sky130::sky130_fd_pr__nfet_01v8_draw {"
            f"w {w_per_finger!s} l {param_l!s} m {param_m!s} nf {param_nf!s}"
            f" diffcov 100 polycov 100"
            f" guard {guard!s} glc {glc!s} grc {grc!s} gtc {gtc!s} gbc {gbc!s}"
            " tbcov 100 rlcov 100"
            f" topc {topc!s} botc {botc!s}"
    ...
    def gen_sky130_fd_pr__res_xhigh_po_0p35(d: ThreeTermResistor) -> str:
        ...
        command = (
            ...
            f"w {param_w!s} l {param_l!s} m 1 nx {param_m!s} "
    ...
    def gen_sky130_fd_pr__cap_mim_m3_1(d: Capacitor) -> str:
        ...
        # generate the command
        command = (
            "::sky130::sky130_fd_pr__cap_mim_m3_1_draw { w "
            + str(param_w)
            + " l "
            + str(param_l)
            + " nx "
    ...
    def gen_sky130_fd_pr__cap_mim_m3_2(d: Capacitor) -> str:
        ...
        # generate the command
        command = (
            "::sky130::sky130_fd_pr__cap_mim_m3_2_draw { w "
            + str(param_w)
            + " l "
            + str(param_l)
            + " nx "
