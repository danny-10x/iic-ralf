...
class PlacementRules:
    ...
    def generate_rule(
        self, other_rules: PlacementRules
...
class PlacementRule(metaclass=abc.ABCMeta):
    ...
    def generate_rule(self, other_rule: PlacementRule):
        """Get the bounding box of the cell, which satisfies the rules.
...
class Spacing(PlacementRule):
    ...
    def conflict(self, other_rule: PlacementRule) -> bool:
        ...
        A spacing conflict appears, if the other rule is also a spacing rule and they
        have the same layer but different nets.
    ...
    def generate_rule(
        self, other_rule: PlacementRule
    ) -> tuple[float, float, float, float]:
        """Get the bounding box of the cell, which satisfies the spacing rule.
...
class MacroSpacing(Spacing):
    ...
    def __init__(
        self, *, cell: MacroCell, cell_spacing: Spacing, net: Net = None
    ) -> None:
        """Initialize a spacing rule for a macro cell.
        ...
        cell_bound_enlarged = [
            cell_bound[0] - cell_min_space,
            cell_bound[1] - cell_min_space,
            cell_bound[2] + cell_min_space,
            cell_bound[3] + cell_min_space,
        ]
        ...
        self._spaces = [
            max(
                macro_bound[0] - cell_bound_enlarged[0], 0
            ),  # needed spacing at the W side of the macro
            max(
                macro_bound[1] - cell_bound_enlarged[1], 0
            ),  # needed spacing at the S side of the macro
            max(
                cell_bound_enlarged[2] - macro_bound[2], 0
            ),  # needed spacing at the E side of the macro
            max(cell_bound_enlarged[3] - macro_bound[3], 0),
        ]  # needed spacing at the N side of the macro
