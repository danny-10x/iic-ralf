...
from magic.magic_layer import Rectangle
from magic.magic_terminal import MagicPin, MagicTerminal
from schematic_capture.devices import MOS, Capacitor, PrimitiveDevice, ThreeTermResistor
from schematic_capture.primitives import (
    CrossCoupledPair,
    DifferentialLoad,
    DifferentialPair,
)
from schematic_capture.rstring import RString
...
def get_terminals_mos(cell: Cell) -> MagicTerminal:
    ...
    """
    device = cell.device
    assert isinstance(device, MOS), "Device isn't a MOS!"
    assert device.parameters["m"] == 1, "Device isn't build up by a single MOS!"
    ...
    assert len(gate_rects) == device.parameters["nf"], (
        "Number of devices gates, and found gates don't match!"
    )
    ...
    if rot == 0 or rot == 180:
        ...
        drain_source_rects = sorted(
            drain_source_rects, key=lambda rectangles: rectangles.bounding_box[0]
        )
        ...
        drain_source_rects = merge_rects2(drain_source_rects)
        drain_source_rects = sorted(
            drain_source_rects, key=lambda rectangles: rectangles.bounding_box[1]
        )
        # bulk_rects = merge_rects(bulk_rects, direction=0)
        bulk_rects = merge_rects2(bulk_rects)

    assert len(drain_source_rects) == (device.parameters["nf"] + 1), (
        "Devices number of drain/source and found drain/source contacts don't match!"
    )
    assert len(bulk_rects) == 1, "More than one bulk contact found!"
    ...
    if source_bulk_connected:  # if source and bulk are connected
        if (device.parameters["nf"] % 2) == 0:  # if the number of fingers is even
            ...
            if ((device.parameters["nf"] // 2) % 2 == 0 and not device.use_dummies) or (
                (device.parameters["nf"] // 2) % 2 == 1 and device.use_dummies
            ):  # if a drain-node is at the center of the device
                drain_rects, source_rects = (
                    source_rects,
                    drain_rects,
                )  # change drain and source
    ...
    terminals = generate_terminals(
        terminal_rects, terminal_layers, terminal_location, cell
    )
    return terminals


def get_terminals_three_term_resistor(cell: Cell) -> dict[str, MagicTerminal]:
    """Map the terminals D,S,B to terminals/pins of the devices cell, for a three-terminal-resistor.
    ...
    """
    device = cell.device
    assert isinstance(device, ThreeTermResistor), "Device isn't a ThreeTermResistor!"
    assert device.parameters["m"] == 1, "Device isn't build up by a single resistor!"
    assert not device.use_dummies, "Dummy-Resistors aren't supported!"

    rot = cell.rotation
    assert rot == 0, "Rotation different, than 0deg detected!"
    ...
    drain_source_rects = sorted(
        drain_source_rects,
        key=lambda rectangles: (rectangles.bounding_box[1], rectangles.bounding_box[0]),
    )
    bulk_rects = merge_rects2(bulk_rects)

    assert len(bulk_rects) == 1, "More than one bulk-contact detected!"
    assert len(drain_source_rects) == 2, (
        "More than two drain-source contact for a single resistor detected!"
    )
...
def get_terminals_capacitor(cell: Cell) -> dict[str, MagicTerminal]:
    """Map the terminals D,S to terminals/pins of the devices cell, for a capacitor.
    ...
    """
    device = cell.device
    assert isinstance(device, Capacitor), "Device isn't a Capacitor!"
    assert device.parameters["m"] == 1, "Device isn't build up by one Capacitor!"
    assert not device.use_dummies, "Dummy-Capacitors aren't supported!"
    rot = cell.rotation
    assert rot == 0, "Rotation different, than 0deg detected!"
...
def get_terminals_differential_pair(cell: Cell) -> dict[str, MagicTerminal]:
    """Map the terminals G1, G2, D1, D2, S, B to terminals/pins of the devices cell.
    ...
    assert isinstance(device, DifferentialPair)
    assert device.parameters["m"] == 1, "Device isn't build up by one MOS!"
    ...
    if rot == 0 or rot == 180:
        ...
        gate_rects = merge_rects2(gate_rects)
        gate_rects = sorted(
            gate_rects, key=lambda rectangles: rectangles.bounding_box[0]
        )
    else:
        # gate_rects = merge_rects(gate_rects, direction=0)
        gate_rects = merge_rects2(gate_rects)
        gate_rects = sorted(
            gate_rects, key=lambda rectangles: rectangles.bounding_box[1]
        )
    ...
    if botc == 0:
        ...
        assert len(gate_rects) == 2 * (device.parameters["nf"]), (
            "Number of devices gates, and found gates don't match!"
        )
    ...
    assert len(bulk_rects) == 1, "More than one bulk contact detected!"
    assert len(drain_source_rects) == (device.parameters["nf"] + 1), (
        "Number of devices drain/source contacts and found don't match!"
    )
    ...
    if index_half % 2 == 0:  # nf per device even
        ...
        if rot == 0 or rot == 180:
            drain_rects = sorted(
                drain_rects, key=lambda rectangles: rectangles.bounding_box[0]
            )
        else:
            drain_rects = sorted(
                drain_rects, key=lambda rectangles: rectangles.bounding_box[1]
            )
...
def get_terminals_differential_load(cell: Cell) -> dict[str, MagicTerminal]:
    """Map the terminals D1,D2,S1,S2,G,B to terminals/pins of the devices cell, for a differential load.
    ...
    assert isinstance(device, DifferentialLoad)
    assert device.parameters["m"] == 2, "Device isn't build up by two MOS!"
    ...
    if rot == 0 or rot == 180:
        ...
        gate_rects = sorted(
            ...
            key=lambda rectangles: (
                rectangles.bounding_box[1],
                rectangles.bounding_box[0],
            ),
        )
    else:
        # gate_rects = merge_rects(gate_rects, direction=0)
    ...
    if botc == 0:
        # device don't uses bottom-gate contacts
        assert len(gate_rects) == (device.parameters["nf"] * 2), (
            "Number of devices gates, and found gates don't match!"
        )
    else:
        # device uses bottom-gate contacts
        assert len(gate_rects) == 2 * (device.parameters["nf"] * 2), (
            "Number of devices gates, and found gates don't match!"
        )
    ...
    assert len(bulk_rects) == 1, "More than one bulk contact detected!"
    assert len(drain_source_rects) == 2 * (device.parameters["nf"] + 1), (
        "Number of devices drain/source contacts and found don't match!"
    )
...
def get_terminals_cross_coupled_pair(cell: Cell) -> dict[str, MagicTerminal]:
    """Get terminals of cross couple pair.
    ...
    """
    device = cell.device
    assert isinstance(device, CrossCoupledPair), "Device isn't a cross-coupled-pair!"
    assert device.parameters["m"] == 2, "Device isn't build up by two MOS!"
    rot = cell.rotation
    assert rot == 0, "Rotation different, than 0deg detected!"
...
def get_terminals_rstring(cell: Cell) -> dict[str, MagicTerminal]:
    """Map the terminals of the RString to terminals/pins of the devices cell.
    ...
    assert isinstance(rstring_device, RString), (
        f"Cells device isn't a RString! ({cell})"
    )
    ...
    drain_source_rects = merge_rects2(drain_source_rects)
    assert len(drain_source_rects) == rstring_device.parameters["m"] * 2, (
        "Number of devices and pin-areas don't match!"
    )

    # sort the rectangles according their lower-left corner, in x-direction
    drain_source_rects = sorted(
        drain_source_rects,
        key=lambda rectangles: (rectangles.bounding_box[0], rectangles.bounding_box[1]),
    )

    # bulk_rects = merge_rects(bulk_rects, direction=0)
    bulk_rects = merge_rects2(bulk_rects)

    assert len(bulk_rects) == 1, "More than one Bulk-rectangle generated for a RString"
    assert len(drain_source_rects) == (2 * rstring_device.parameters["m"]), (
        "Number of drain/source rectangles don't match with number of resistors!"
    )
    ...
    for i in range(1, len(drain_source_rects) // 2, 2):
        drain_source_rects[i * 2], drain_source_rects[i * 2 + 1] = (
            drain_source_rects[i * 2 + 1],
            drain_source_rects[i * 2],
        )
    ...
    if uses_dummies:
        ...
        n_resistors = len(drain_source_rects) // 2
...
def generate_terminals(
    terminal_rects: dict[str, list[Rectangle]],
    terminal_layers: dict[str, str],
    terminal_location: dict[str, str],
    cell: Cell,
) -> dict[str, MagicTerminal]:
    ...
    for terminal, rects in terminal_rects.items():
        device = cell.device
        assert isinstance(device, PrimitiveDevice), (
            "A physical terminal, can only be generated for a primitive device."
        )
        ...
        pin_locations = map_rects_to_terminal_location(
            rects, terminal_location[terminal], cell.rotation
        )
        # get the PDK layer
...
def transform_rects_to_lambda(rects: list[Rectangle]) -> list[Rectangle]:
    ...
    for rect in rects:
        ...
        x_min = math.floor(rect.bounding_box[0] / 2.0) * 2
        y_min = math.floor(rect.bounding_box[1] / 2.0) * 2

        x_max = math.ceil(rect.bounding_box[2] / 2.0) * 2
        y_max = math.ceil(rect.bounding_box[3] / 2.0) * 2
...
def map_rects_to_terminal_location(
    terminal_rects: list[Rectangle], terminal_location: str, rotation: int
) -> list[tuple[int, int]]:
    """Map the rectangular-pin area to the x-y coordinate of a pin.
