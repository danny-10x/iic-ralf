...
if TYPE_CHECKING:
    from rules.placement_rules import PlacementRules
...
class Cell:
    ...
    def __init__(
        self, name: str, layer_stack: dict[str, MagicLayer], device: Device = None
    ):
        """Initialize a cell-view of a device.
    ...
    def add_terminals(self):
        ...
        if self._device:
            try:
                ...
                raise ValueError(
                    f"Terminals for device {self.device.name} not implemented!"
                ) from e
        else:
    ...
    def place_next(self):
        """Set the `in_placement` attribute, to signal that the cell gets placed as next."""
    ...
    def get_overlapping_rectangles(self, layer1: str, layer2: str) -> list[Rectangle]:
        ...
        try:
            ...
            raise ValueError(
                f"Layer {layer1} not in layer-stack of cell {self._name}."
            ) from e
        ...
        try:
            ...
            raise ValueError(
                f"Layer {layer2} not in layer-stack of cell {self._name}."
            ) from e

        return MagicLayer.get_overlaps(l1, l2)

    def get_placement_bounding_box(
        self, other_cell: Cell = None
    ...
    def _move_layers_to_bounding(self):
        """Move the layers of the cell, such that they match with the moved bounding box."""
        layers = []
        ...
        # rotate the layers
        for layer_ in layers:
            dphi = (self.rotation - layer_.rotation) % 360
            if not dphi == 0:
                layer_.rotate((mean_x, mean_y), dphi)

        # move the layers to the bounding box
        for layer_ in layers:
            layer_.move((self.center_point[0] - mean_x, self.center_point[1] - mean_y))
    ...
    def draw(self, screen):
        ...
        # draw each layer
        for layer_ in list(self._layer_stack.values()):
            layer_.draw(screen)
    ...
    def plot(self, ax, text: bool = True, with_terminals=False):
        ...
        if text:
            ...
            probs = {"boxstyle": "round", "facecolor": "k", "alpha": 0.1}
            ax.text(
                *mean,
                self._name,
                fontsize=14,
    ...
    def _move(self, coordinate: tuple[int | float, int | float]):
        """Move the boundary and physical terminals of the cell, by the amount given in coordinate.
        ...
        for _, term in self._terminals.items():
    ...
    def _rotate(self, coordinate: tuple[int | float, int | float], angle: int):
        """Rotate the bounding-layer and the terminals clockwise by the angle <angle>, about the coordinate <coordinate>.
    ...
    def rotate_center(self, angle: int):
        """Rotate the bounding-layer and the terminals clockwise by the angle <angle>, about the center point of the cell.
