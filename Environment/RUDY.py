...
class Rudy:
    ...
    def __init__(self, pdk: PDK) -> None:
        ...
        self._nets: dict[
            str, tuple
        ] = {}  # key: net-name, value: (wire_density, bounding_box)
    ...
    def calc_routing_demand(self, x: float, y: float):
        """Calculate the routing demand at coordinate (x,y).
        ...
        Returns:
            float: Routing demand.

        """
        demand = 0
        for _, (d, bound) in self._nets.items():
    ...
    def calc_routing_supply(self, x, y):
        """Calculate the routing supply, at coordinate (x,y).
        ...
        Returns:
            float: Routing supply.

        """
        return 1.0

    def calc_routing_demand_in_area(
        self, x_min, y_min, x_max, y_max, n_x=64, n_y=64
    ) -> np.ndarray:
        ...
        for i in range(n_x):
            for j in range(n_y):
                demand[j, i] = self.calc_routing_demand(x[i], y[j])
    ...
    def plot_route_demand(self, x_min, y_min, x_max, y_max, n_x=64, n_y=64):
        ...
        """
        demand = self.calc_routing_demand_in_area(x_min, y_min, x_max, y_max, n_x, n_y)

        plt.imshow(demand, cmap="hot", origin="lower")
        plt.colorbar()
