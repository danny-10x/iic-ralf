...
class Placement:
    ...
    def __init__(
        ...
        """Initialize a placement environment for a RL problem.
        ...
        # Reward defs.
        self._DRC_error_rew = (
            -(self._size[0] + self._size[1]) * self._n_devices
        )  # worst possible HPWL
    ...
    def init_circuit(self):
        ...
        for _, d in self._circuit._devices.items():
            d.cell.reset_place()
        ...
        if self._data:
            ...
            if update_only:
                ...
                for d in update_only:
                    ...
                    data.x[d_index] = tensor(
                        self._circuit.devices[d_name].feature_list, dtype=float32
                    )
                    ...
                    for i in indices:
                        ...
                        data.edge_attr[i] = tensor(
                            self._circuit.get_edge_feature(name1, name2, net_name),
                            dtype=float32,
                        )
            else:
                ...
                data.x = tensor(
                    [self._circuit.devices[d].feature_list for d in data.name],
                    dtype=float32,
                )
                ...
                data.edge_attr = tensor(
                    [
                        self._circuit.get_edge_feature(
                            node_names[u], node_names[v], net
                        )
                        for (u, v, net) in zip(nodes1, nodes2, edge_nets)
                    ],
                    dtype=float32,
                )
        else:
            ...
            data.x = data.x.to(dtype=float32)
            data.edge_attr = data.edge_attr.to(dtype=float32)
            self._data = data
    ...
    def step(
        self, coordinate: tuple[int, int], rotation: int
    ) -> tuple[torch_geometric.data.Data, float, bool, bool, str]:
        ...
        if len(self._placement_order) == 0:  # Placing finished
            ...
            self.HPWLs.insert(0, total_wire_len)
