...
def do_placement(
    ...
    name: str,
    n_placements: int,
    placements_per_rollout=100,
    use_weights=False,
    show_stats=True,
):
    """Perform the placement of circuit <circ>.
    ...
    Args:
        ...
        show_stats (bool): If Ture, show placement stats.

    Returns:
        Circuit: Circuit of best placement.

    """
    assert n_placements % placements_per_rollout == 0, (
        "Number of total placements not divisible by the number of placements per rollout!"
    )
    ...
    logger.info(
        f"Doing placement {name} of circuit {circ.name} for {n_rollouts} rollouts."
    )
    logger.debug(f"Circuit: type {type(circ)}, id {id(circ)}")

    # get maximum side-length of the placement
    side_length = 0
    for _, device_ in circ.devices.items():
        assert isinstance(device_, Device)
    ...
    logger.debug(
        f"Circuit of best placement:  type {type(best_placement)}, id {id(best_placement)}"
    )

    print(
        f"Placement of {name} done with reward {best_rew}, found at placement #{best_placement_number}."
    )
    ...
    if show_stats:
        ...
        for device_ in best_placement.devices.values():
            bound = device_.cell.get_bounding_box()
...
def do_bottom_up_placement(
    circuit: Circuit,
    n_placements: int,
    placements_per_rollout: int = 100,
    use_weights=False,
    show_stats=True,
):
    ...
    """
    logger.info(
        f"Performing bottom-up placement on circuit {circuit.name} for {n_placements} placements per circuit/subcircuit."
    )
    logger.debug(f"Circuit: type {type(circuit)}, id {id(circuit)}")
    ...
    for _, circuit_ in placement_order:
        if circuit_.name in circuit_dict:
            # if circuit was already placed, get the circuit
            best_circuit = circuit_dict[circuit_.name]
        else:
            # place circuit
            if len(circuit_.devices) > 1:
                best_circuit = do_placement(
                    ...
                    placements_per_rollout=placements_per_rollout,
                    use_weights=use_weights,
                    show_stats=show_stats,
                )
                circuit_dict[circuit_.name] = copy.deepcopy(best_circuit)
                logger.debug(
                    f"Best-circuit: type {type(best_circuit)}, id {id(best_circuit)}"
                )
            else:
        ...
        for k, v in best_circuit_mapped.items():
            ...
            logger.debug(
                f"Best-location {v.cell.center_point}, Best-rotation {v.cell.rotation}."
            )
            circuit_mapped[k].cell.reset_place()
            circuit_mapped[k].cell.place(v.cell.center_point, v.cell.rotation)
            logger.debug(
                f"Updated cell to location {circuit_mapped[k].cell.center_point} and rotation {circuit_mapped[k].cell.rotation}."
            )

        # c.devices = best_circuit.devices

        if type(circuit_) is SubCircuit:
            # if circuit was a subcircuit, make a macro cell out of the placed cells
            logger.debug(f"Adding macrocell to subcircuit {circuit_.name}.")
            ...
            logger.debug(
                f"Created macrocell {macro._name}, type {type(macro)}, id {id(macro)}."
            )

            circuit_.sub_device.set_cell(macro)

    for t, circuit_ in reversed(placement_order):
