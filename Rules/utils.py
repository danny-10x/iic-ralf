    ...
    if os.path.isfile(file) and file.endswith(".json"):
        ...
        for rule, args in data:
            ...
            try:
                ...
                raise NotImplementedError(f"Rule {rule} isn't a valid net-rule!") from e
    ...
    if len(name_splitted) > 1:  # net is a subnet
        for sub_dev_name in name_splitted[:-1]:
            try:
                ...
                raise ValueError(
                    f"SubDevice {sub_dev_name} isn't in circuit {sub_circ.name}!"
                ) from e
            try:
                # get the circuit of the sub-device
                sub_circ = sub_dev._circuit
            except Exception as e:
                raise ValueError(f"Device {sub_dev} has no circuit!") from e

    try:
        net = sub_circ._nets[name_splitted[-1]]
    except Exception as e:
        raise ValueError(
            f"Net {name_splitted[-1]} isn't in circuit {sub_circ.name}!"
        ) from e
